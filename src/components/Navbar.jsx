...
const Navbar = ({ theme, setTheme }) => {
  ...
  return (
    ...
    ">
      ...
      <div className={`text-gray-700 dark:text-white sm:text-sm ${!sidebarOpen ? 'max-sm:w-0 overflow-hidden' : 'max-sm:w-60 max-sm:pl-10'} max-sm:fixed top-0 bottom-0 right-0 max-sm:min-h-screen max-sm:h-full max-sm:flex-col max-sm:bg-blue-600 max-sm:text-white max-sm:pt-20 flex sm:items-center gap-5 transition-all`}>
      ...
      <div className='flex items-center gap-2 sm:gap-4'>
        ...
        <a
          href="#contact-us"
          className='text-sm max-sm:hidden flex items-center gap-2 bg-blue-500 text-white px-6 py-2 rounded-full cursor-pointer hover:scale-105 transition-all'
        >
          Connect
